...
class PlayerCPUAdaptive(FES.Player):
    def __init__(self, playerID: int, isInverted: bool, history_path: Path | None = None) -> None:
        super().__init__(playerID, isInverted)
    ...
    def _load_history(self) -> dict:
        ...
        try:
            ...
            if "totals" not in data or "transitions" not in data:
                return self._default_history()
            return data
    ...
    def record_player_choice(self, finger: str) -> None:
        if finger not in FES.fingerNames:
            return
        self.history["totals"][finger] += 1
        if self.last_player_finger:
            self.history["transitions"][self.last_player_finger][finger] += 1
    ...
    def record_game_result(self, winner_id: int) -> None:
        ...
        if winner_id == 1:
            self.history["games"]["player_wins"] += 1
        elif winner_id == 2:
            self.history["games"]["cpu_wins"] += 1
        self._save_history()
    ...
    def chooseFinger(self) -> None:
        ...
        for finger in FES.fingerNames:
            ...
            if self.last_player_finger:
                base += transitions.get(self.last_player_finger, {}).get(finger, 0) * 2
            weights[finger] = base
